...
from splatkit_native_backends.faster_gs import render_faster_gs_native
from splatkit_native_backends.faster_gs.renderer import (
    FasterGSNativeRenderOptions,
    _split_sh_coefficients,
    _validate_inputs,
)
from splatkit_native_backends.gaussian_pop.runtime import (
...
def render_gaussian_pop(
    ...
    *,
    ...
    if return_projective_intersection_transforms:
        raise ValueError(
            "The gaussian_pop backend does not expose projective "
    ...
    if not return_gaussian_impact_score:
        if return_depth:
            depth_result = render_faster_gs_depth(
                scene,
                camera,
                return_depth=True,
