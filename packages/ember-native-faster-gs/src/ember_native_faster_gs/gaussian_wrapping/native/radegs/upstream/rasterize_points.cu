	...
	const bool debug)
{
  ...
  torch::Tensor out_depth_sum = torch::full({1, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_color_square.contiguous().data<float>(),
		out_depth_sum.contiguous().data<float>(),
		out_depth_square.contiguous().data<float>(),
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
 RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& dL_dout_depth_sum,
	...
	const bool debug) 
{
  ...
  {  
	  ...
	  dL_dout_color_square.contiguous().data<float>(),
	  dL_dout_depth_sum.contiguous().data<float>(),
	  dL_dout_depth_square.contiguous().data<float>(),
