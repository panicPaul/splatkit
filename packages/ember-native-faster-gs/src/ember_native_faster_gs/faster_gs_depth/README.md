...
1. [`renderer.py`](./renderer.py)
   This is the ember-core-facing adapter. It handles backend options and packages RGB plus
   depth outputs.
...
- Override only the blend pair because this backend needs extra depth outputs and depth
  gradients.
- Keep the public backend API ember-core-native while keeping the native stage boundaries
  explicit.
