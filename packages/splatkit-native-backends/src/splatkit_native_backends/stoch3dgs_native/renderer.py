"""Splatkit-native Stoch3DGS backend adapter."""
...
from splatkit.core.capabilities import HasAlpha, HasDepth, HasNormals
...
def _flatten_sh_features(scene: GaussianScene3D) -> Tensor:
    ...
    if feature.ndim != 3:
        raise ValueError(
            "stoch3dgs_native expects spherical harmonics with shape "
...
def _validate_inputs(scene: GaussianScene3D, camera: CameraState) -> None:
    if scene.center_position.device.type != "cuda":
        raise ValueError("stoch3dgs_native requires scene tensors on CUDA.")
    if camera.cam_to_world.device.type != "cuda":
        raise ValueError("stoch3dgs_native requires camera tensors on CUDA.")
    if camera.camera_convention != "opencv":
        raise ValueError(
            "stoch3dgs_native currently expects cameras in opencv convention; "
            f"got {camera.camera_convention!r}."
        )
    if scene.log_scales.shape[-1] != 3:
        raise ValueError(
            "stoch3dgs_native only supports 3D Gaussian scales with shape "
    ...
    if not torch.equal(camera.width, camera.width[:1].expand_as(camera.width)):
        raise ValueError("stoch3dgs_native requires a uniform image width across cameras.")
    if not torch.equal(camera.height, camera.height[:1].expand_as(camera.height)):
        raise ValueError("stoch3dgs_native requires a uniform image height across cameras.")
...
def render_stoch3dgs_native(
    ...
    *,
    ...
    if return_gaussian_impact_score:
        raise ValueError(
            "stoch3dgs_native does not expose Gaussian impact scores."
        )
    if return_2d_projections:
        raise ValueError("stoch3dgs_native does not expose 2D Gaussian projections.")
    if return_projective_intersection_transforms:
        raise ValueError(
            "stoch3dgs_native does not expose projective intersection transforms."
        )
