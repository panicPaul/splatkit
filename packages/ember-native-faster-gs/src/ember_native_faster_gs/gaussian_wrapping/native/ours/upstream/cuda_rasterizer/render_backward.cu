...
__global__ void __launch_bounds__(BLOCK_X* BLOCK_Y)
    renderCUDA(
        ...
        const float* __restrict__ dL_dpixel_color_squares,
        const float* __restrict__ dL_dpixel_depths,
        const float* __restrict__ dL_dpixel_depth_squares,
    ...
    float dL_dpixel[C];
    float dL_dfinalT;
    float accum_color_square_rec = 0.f;
    float grad_color_square = 0.f;
    [[maybe_unused]] float accum_depth_rec = 0.f;
    [[maybe_unused]] float accum_depth_square_rec = 0.f;
    [[maybe_unused]] float grad_depth = 0.f;
    [[maybe_unused]] float grad_depth_square = 0.f;
...
#pragma unroll
        ...
        grad_color_square = dL_dpixel_color_squares[pix_id];
...
#pragma unroll
        ...
        if constexpr (GEOMETRY) {
            ...
            grad_depth = dL_dpixel_depths[pix_id];
            grad_depth_square = dL_dpixel_depth_squares[pix_id];
        ...
        for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
            ...
            for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
                ...
                if (rsigma > 0.f) {
                    ...
                    dT_dtm += -0.5f * dmul * Gt / (1.f - Gt) * fabsf(t_delta) * rsigma;
                }
            }
        }
        ...
        for (int j = 0; j < min(BLOCK_SIZE, toDo); j++) {
            ...
            if (valid) {
                ...
                for (int ch = 0; ch < C; ch++) {
                    ...
                    dL_dcolors_local[ch] +=
                        blending_weight * 2.f * c * grad_color_square;
                }
                accum_color_square_rec =
                    last_alpha * color_square
                    + (1.f - last_alpha) * accum_color_square_rec;
                dL_dopa +=
                    (color_square - accum_color_square_rec) * grad_color_square;
...
#pragma unroll
                    ...
                    if (rsigma > 0.f) {
                        ...
                        const float dmul = (t_delta > 0.f) ? TDELTA_POSITIVE_GRAD_MUL : TDELTA_NEGATIVE_GRAD_MUL;
                        dL_dGt = sign * dmul * dL_dmt_dT_dtm * 0.5f / (1.f - Gt);
                    }
                    dL_dopa_sigma          = dL_dGt * G_exp - dL_dmt_dT_dtm * (t_delta > 0.f ? 0.5f / (1.f - alpha) : 0.f);
                    float dL_ddelta        = -dL_dGt * Gt * t_delta;
                    dL_drsigma_local       = dL_ddelta * (mDepth - t_peak);
                    const float dL_dt_peak = -dL_ddelta * rsigma;

                    const float rln = rnorm3df(pixnf.x, pixnf.y, 1.f);
                    const float depth = t_peak * rln;
                    accum_depth_rec =
                        last_alpha * depth
                        + (1.f - last_alpha) * accum_depth_rec;
                    accum_depth_square_rec =
                        last_alpha * depth * depth
                        + (1.f - last_alpha) * accum_depth_square_rec;
                    dL_dopa += (depth - accum_depth_rec) * grad_depth;
                    dL_dopa +=
                        (depth * depth - accum_depth_square_rec)
                        * grad_depth_square;

                    dL_dt = dL_dt_peak;
                    dL_dt += blending_weight * rln * grad_depth;
                    dL_dt +=
                        blending_weight * 2.f * depth * rln
                        * grad_depth_square;
...
void BACKWARD::render(
    ...
    const float* dL_dpixel_color_square,
    const float* dL_dpixel_depth,
    const float* dL_dpixel_depth_square,
...
#define RENDER_CUDA_CALL(template_depth)                                    \
    renderCUDA<NUM_CHANNELS, template_depth><<<grid, block>>>(              \
        ...
        dL_dpixel_mdepth, dL_dpixel_color_square, dL_dpixel_depth,          \
        dL_dpixel_depth_square, dL_dalphas, dL_dpixel_normals,              \
