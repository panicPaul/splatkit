...
This package is the native FasterGS backend inside the `ember-core` ecosystem.
...
It has two jobs:
- expose a normal ember-core backend entrypoint via [`renderer.py`](./renderer.py)
...
1. [`renderer.py`](./renderer.py)
   This is the ember-core-facing adapter. It validates inputs, loops over cameras, and calls the runtime.
...
- `renderer.py`
  Thin backend adapter. This is where ember-core contracts like `GaussianScene3D` and `CameraState` are handled.
- `runtime/`
...
- `render_faster_gs_native(...)`
  The public package path is now `ember_native_faster_gs.faster_gs`.
- `runtime.preprocess(...)`
- `runtime.sort(...)`
- `runtime.blend(...)`
- `runtime.render(...)`

The raw torch custom ops are registered under the `faster_gs::` namespace and live under [`runtime/ops`](./runtime/ops).

## Design goals

- Keep the backend API ember-core-native.
...
process can fail even if it does not use `ember-core` at all.
