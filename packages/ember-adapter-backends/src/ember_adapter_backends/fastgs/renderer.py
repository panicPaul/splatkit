...
def _import_fastgs_runtime() -> tuple[type[Any], type[Any]]:
    try:
        ...
        raise ImportError(
            ...
            'dependency. Install it with `pip install "ember-adapter-backends[fastgs]"`.'
        ) from exc
...
def render_fastgs(
    ...
    *,
    ...
    if options is not None and options.debug:
        raise ValueError(
            "The FastGS backend debug path is not supported in ember_core."
        )
...
def register() -> None:
    """Register the FastGS backend in the global ember-core registry."""
    register_backend(
        name="adapter.fastgs",
