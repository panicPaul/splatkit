...
template <int ParticleKernelDegree = 4, bool SurfelPrimitive = false>
__device__ inline void processHitStochasticBwd(
    const float3& rayOrigin,
    const float3& rayDirection,
    int32_t particleIdx,
    ...
    float minTransmittance,
    int32_t sphEvalDegree,
    float transmittanceGrad, // transmittanceGrad is supported for stochastic
    float3 radianceGrad, // radianceGrad is supported for stochastic
    // don't support depthGrad for stochastic
    ...
    if ((gres > minParticleKernelDensity) && (galpha > minParticleAlpha)) {
        ...
        const float gsqdist = dot(grds, grds);
        const float gdist   = sqrtf(gsqdist);
        if (gdist > rayDistance && !isHit) {
            // if the distance is larger than the ray distance, we do not need to process this particle
            return;
        }
        
        float transmittanceCoeff = 0.0f;
        
        transmittanceCoeff = -1.0f / (1.0f - galpha);
        // if (galpha < minParticleAlpha) {
        //     transmittanceCoeff = make_float3(0.0f);
        // }
        
        if (isHit) {
            transmittanceCoeff = 1.0f / galpha;
            // transmittanceCoeff = make_float3(0.0f);
        }
        ...
        fetchParticleSphCoefficients(
            ...
            &sphCoefficients[0]);
        if (isHit) {
            float3 grad = radianceFromSpHBwd(sphEvalDegree, &sphCoefficients[0], rayDirection, 1.0f, radianceGrad, 
                                                (float3*)&particleRadianceGradPtr[particleIdx * SPH_MAX_NUM_COEFFS * 3]);
        } 
        

        float densityGrad = (finalRadiance.x * radianceGrad.x +
                            finalRadiance.y * radianceGrad.y +
                            finalRadiance.z * radianceGrad.z + transmittanceGrad) * transmittanceCoeff;
        atomicAdd(
            &particleDensityGrad.density, gres * densityGrad);


        // dL/dalpha = densityGrad; alpha = gres * particleDensity
        // dL/dgres = dL/dalpha * dalpha/dgres
        //           = densityGrad * particleDensity
        const float gresGrd =
            particleDensity * densityGrad;
        ...
        atomicAdd(&particleDensityGrad.quaternion.z, grotGrdPoscr.z + grotGrdRayDirR.z);
        atomicAdd(&particleDensityGrad.quaternion.w, grotGrdPoscr.w + grotGrdRayDirR.w);
        float4 quaternionIncre = make_float4(
            grotGrdPoscr.x + grotGrdRayDirR.x,
            grotGrdPoscr.y + grotGrdRayDirR.y,
            grotGrdPoscr.z + grotGrdRayDirR.z,
            grotGrdPoscr.w + grotGrdRayDirR.w);
...
__device__ inline void processHitStochasticBwd2(
    ...
    if ((gres > minParticleKernelDensity) && (galpha > minParticleAlpha)) {
        ...
        float transmittanceCoeff = 1.0f / galpha;

        float densityGrad = ((grad_cur - grad_res).x * radianceGrad.x +
                            (grad_cur - grad_res).y * radianceGrad.y +
                            (grad_cur - grad_res).z * radianceGrad.z) * transmittanceCoeff;
        atomicAdd(
            &particleDensityGrad.density, gres * densityGrad);
