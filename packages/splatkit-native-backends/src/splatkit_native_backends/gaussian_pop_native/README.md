...
| --- | --- | --- | --- |
| Backend adapter | Yes | `gaussian_pop_native.renderer` | Public `render(...)` entrypoint and output packaging |
| Preprocess stage | No | `faster_gs_native.reuse` | Reuse the root FasterGS preprocess op directly |
| Sort stage | No | `faster_gs_native.reuse` | Reuse the root FasterGS sort op directly |
| Blend forward stage | Yes | `gaussian_pop_native.runtime.blend` | Backend-owned blend stage that reuses root/depth blend ops and adds the GaussianPOP score |
| RGB blend kernels | No | `faster_gs_native.reuse` | Root-owned RGB blend op reused inside the GaussianPOP blend stage |
| Expected-depth blend kernels | No | `faster_gs_depth_native.reuse` | Depth-native blend op reused inside the GaussianPOP blend stage |
| Gaussian impact score | Yes | `gaussian_pop_native.runtime.blend` | Forward-only score computed inside the backend-owned blend stage |
| Native score helper | Yes | `gaussian_pop_native.native` | Backend-owned CUDA helper for GaussianPOP score accumulation |
