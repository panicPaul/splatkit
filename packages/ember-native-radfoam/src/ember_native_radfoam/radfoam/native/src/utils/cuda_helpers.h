...
inline void global_cuda_init() {
    cuda_check(cuInit(0));
    cudaError_t heap_limit_status =
        cudaDeviceSetLimit(cudaLimitMallocHeapSize, 1ul << 29ul);
    if (heap_limit_status != cudaSuccess) {
        // The upstream module may already have initialized CUDA in this process.
        // Keep import idempotent so reference-parity tests can load both modules.
        cudaGetLastError();
    }
}
