"""GaussianPOP-style FasterGS native backend adapter."""

from __future__ import annotations

from dataclasses import dataclass
from typing import Literal, overload

import torch
from beartype import beartype
from jaxtyping import Float
from splatkit.core.capabilities import HasDepth, HasGaussianImpactScore
from splatkit.core.contracts import (
    CameraState,
    GaussianScene3D,
    RenderOptions,
    RenderOutput,
)
from splatkit.core.registry import register_backend
from torch import Tensor

from splatkit_native_backends.faster_gs_depth_native import (
    render_faster_gs_depth_native,
)
from splatkit_native_backends.faster_gs_depth_native.renderer import (
    FasterGSDepthNativeRenderOptions,
)
from splatkit_native_backends.faster_gs_native import render_faster_gs_native
from splatkit_native_backends.faster_gs_native.renderer import (
    FasterGSNativeRenderOptions,
    _split_sh_coefficients,
    _validate_inputs,
)
from splatkit_native_backends.gaussian_pop_native.runtime import (
    render as render_runtime,
)

_SUPPORTED_OUTPUTS = frozenset({"depth", "gaussian_impact_score"})
DepthTensor = Float[Tensor, " num_cams height width"]
GaussianImpactScoreTensor = Float[Tensor, " num_cams num_splats"]


@beartype
@dataclass(frozen=True)
class GaussianPopNativeRenderOutput(RenderOutput):
    """Base render output for the GaussianPOP native backend."""


@beartype
@dataclass(frozen=True)
class GaussianPopNativeDepthRenderOutput(
    GaussianPopNativeRenderOutput,
    HasDepth,
):
    """Depth-capable GaussianPOP native render output."""

    depth: DepthTensor


@beartype
@dataclass(frozen=True)
class GaussianPopNativeGaussianImpactScoreRenderOutput(
    GaussianPopNativeRenderOutput,
    HasGaussianImpactScore,
):
    """GaussianPOP native render output with per-Gaussian impact scores."""

    gaussian_impact_score: GaussianImpactScoreTensor


@beartype
@dataclass(frozen=True)
class GaussianPopNativeDepthGaussianImpactScoreRenderOutput(
    GaussianPopNativeDepthRenderOutput,
    HasGaussianImpactScore,
):
    """Depth-capable GaussianPOP native render output with impact scores."""

    gaussian_impact_score: GaussianImpactScoreTensor


@beartype
@dataclass(frozen=True)
class GaussianPopNativeRenderOptions(RenderOptions):
    """Render configuration for the GaussianPOP native backend."""

    near_plane: float = 0.01
    far_plane: float = 1000.0
    proper_antialiasing: bool = False


@overload
def render_gaussian_pop_native(
    scene: GaussianScene3D,
    camera: CameraState,
    *,
    return_alpha: Literal[False] = False,
    return_depth: Literal[False] = False,
    return_gaussian_impact_score: Literal[False] = False,
    return_normals: Literal[False] = False,
    return_2d_projections: Literal[False] = False,
    return_projective_intersection_transforms: Literal[False] = False,
    options: GaussianPopNativeRenderOptions | None = None,
) -> GaussianPopNativeRenderOutput: ...


@overload
def render_gaussian_pop_native(
    scene: GaussianScene3D,
    camera: CameraState,
    *,
    return_alpha: Literal[False] = False,
    return_depth: Literal[True] = True,
    return_gaussian_impact_score: Literal[False] = False,
    return_normals: Literal[False] = False,
    return_2d_projections: Literal[False] = False,
    return_projective_intersection_transforms: Literal[False] = False,
    options: GaussianPopNativeRenderOptions | None = None,
) -> GaussianPopNativeDepthRenderOutput: ...


@overload
def render_gaussian_pop_native(
    scene: GaussianScene3D,
    camera: CameraState,
    *,
    return_alpha: Literal[False] = False,
    return_depth: Literal[False] = False,
    return_gaussian_impact_score: Literal[True] = True,
    return_normals: Literal[False] = False,
    return_2d_projections: Literal[False] = False,
    return_projective_intersection_transforms: Literal[False] = False,
    options: GaussianPopNativeRenderOptions | None = None,
) -> GaussianPopNativeGaussianImpactScoreRenderOutput: ...


@overload
def render_gaussian_pop_native(
    scene: GaussianScene3D,
    camera: CameraState,
    *,
    return_alpha: Literal[False] = False,
    return_depth: Literal[True] = True,
    return_gaussian_impact_score: Literal[True] = True,
    return_normals: Literal[False] = False,
    return_2d_projections: Literal[False] = False,
    return_projective_intersection_transforms: Literal[False] = False,
    options: GaussianPopNativeRenderOptions | None = None,
) -> GaussianPopNativeDepthGaussianImpactScoreRenderOutput: ...


@beartype
def render_gaussian_pop_native(
    scene: GaussianScene3D,
    camera: CameraState,
    *,
    return_alpha: bool = False,
    return_depth: bool = False,
    return_gaussian_impact_score: bool = False,
    return_normals: bool = False,
    return_2d_projections: bool = False,
    return_projective_intersection_transforms: bool = False,
    options: GaussianPopNativeRenderOptions | None = None,
) -> (
    GaussianPopNativeRenderOutput
    | GaussianPopNativeDepthRenderOutput
    | GaussianPopNativeGaussianImpactScoreRenderOutput
    | GaussianPopNativeDepthGaussianImpactScoreRenderOutput
):
    """Render a scene with the GaussianPOP-style FasterGS backend."""
    if return_alpha:
        raise ValueError(
            "The gaussian_pop_native backend does not expose alpha output."
        )
    if return_normals:
        raise ValueError(
            "The gaussian_pop_native backend does not expose normals."
        )
    if return_2d_projections:
        raise ValueError(
            "The gaussian_pop_native backend does not expose 2D projections."
        )
    if return_projective_intersection_transforms:
        raise ValueError(
            "The gaussian_pop_native backend does not expose projective "
            "intersection transforms."
        )

    options = options or GaussianPopNativeRenderOptions()
    if not return_gaussian_impact_score:
        if return_depth:
            depth_result = render_faster_gs_depth_native(
                scene,
                camera,
                return_depth=True,
                options=FasterGSDepthNativeRenderOptions(
                    background_color=options.background_color,
                    near_plane=options.near_plane,
                    far_plane=options.far_plane,
                    proper_antialiasing=options.proper_antialiasing,
                ),
            )
            return GaussianPopNativeDepthRenderOutput(
                render=depth_result.render,
                depth=depth_result.depth,
            )
        rgb_result = render_faster_gs_native(
            scene,
            camera,
            options=FasterGSNativeRenderOptions(
                background_color=options.background_color,
                near_plane=options.near_plane,
                far_plane=options.far_plane,
                proper_antialiasing=options.proper_antialiasing,
            ),
        )
        return GaussianPopNativeRenderOutput(render=rgb_result.render)

    _validate_inputs(scene, camera)
    sh_coefficients_0, sh_coefficients_rest = _split_sh_coefficients(scene)
    intrinsics = camera.get_intrinsics()
    background_color = options.background_color.to(
        device=scene.center_position.device,
        dtype=scene.center_position.dtype,
    )

    renders: list[Tensor] = []
    depths: list[Tensor] = []
    gaussian_impact_scores: list[Tensor] = []
    for camera_index in range(camera.cam_to_world.shape[0]):
        cam_to_world = camera.cam_to_world[camera_index]
        world_2_camera = torch.linalg.inv(cam_to_world)
        camera_intrinsics = intrinsics[camera_index]
        render_result = render_runtime(
            scene.center_position.contiguous(),
            scene.log_scales.contiguous(),
            scene.quaternion_orientation.contiguous(),
            scene.logit_opacity[:, None].contiguous(),
            sh_coefficients_0.contiguous(),
            sh_coefficients_rest.contiguous(),
            world_2_camera.contiguous(),
            cam_to_world[:3, 3].contiguous(),
            near_plane=options.near_plane,
            far_plane=options.far_plane,
            width=int(camera.width[camera_index].item()),
            height=int(camera.height[camera_index].item()),
            focal_x=float(camera_intrinsics[0, 0].item()),
            focal_y=float(camera_intrinsics[1, 1].item()),
            center_x=float(camera_intrinsics[0, 2].item()),
            center_y=float(camera_intrinsics[1, 2].item()),
            bg_color=background_color,
            proper_antialiasing=options.proper_antialiasing,
            active_sh_bases=int(scene.feature.shape[1]),
            return_depth=return_depth,
            return_gaussian_impact_score=True,
        )
        renders.append(render_result.image.permute(1, 2, 0).contiguous())
        assert render_result.gaussian_impact_score is not None
        gaussian_impact_scores.append(render_result.gaussian_impact_score.contiguous())
        if render_result.depth is not None:
            depths.append(render_result.depth.contiguous())

    stacked_render = torch.stack(renders, dim=0).clamp(0.0, 1.0)
    stacked_scores = torch.stack(gaussian_impact_scores, dim=0)
    if return_depth:
        return GaussianPopNativeDepthGaussianImpactScoreRenderOutput(
            render=stacked_render,
            depth=torch.stack(depths, dim=0),
            gaussian_impact_score=stacked_scores,
        )
    return GaussianPopNativeGaussianImpactScoreRenderOutput(
        render=stacked_render,
        gaussian_impact_score=stacked_scores,
    )


def register() -> None:
    """Register the GaussianPOP native backend."""
    register_backend(
        name="gaussian_pop_native",
        default_options=GaussianPopNativeRenderOptions(),
        accepted_scene_types=(GaussianScene3D,),
        supported_outputs=_SUPPORTED_OUTPUTS,
    )(render_gaussian_pop_native)
