...
// Implements the native FastGS blend wrappers.
...
namespace fastgs_blend_forward_kernels = ember_faster_gs_core_vendor::rasterization::kernels::forward;
namespace fastgs_blend_backward_kernels =
    ember_faster_gs_core_vendor::rasterization::kernels::backward;
namespace fastgs_blend_config = ember_faster_gs_core_vendor::rasterization::config;
...
blend_fwd_wrapper(
    ...
    const int grid_width = div_round_up(width, fastgs_blend_config::tile_width);
    const int grid_height = div_round_up(height, fastgs_blend_config::tile_height);
    ...
    torch::Tensor tile_final_transmittances = torch::empty(
        {tile_count * fastgs_blend_config::block_size_blend},
        float_options
    );
    ...
    torch::Tensor bucket_color_transmittance = torch::empty(
        {bucket_total * fastgs_blend_config::block_size_blend, 4},
        float_options
    );
    ...
    fastgs_blend_forward_kernels::blend_cu<<<dim3(grid_width, grid_height, 1),
                                dim3(fastgs_blend_config::tile_width, fastgs_blend_config::tile_height, 1)>>>(
    ...
    CHECK_CUDA(fastgs_blend_config::debug, "blend_fwd")
...
torch::Tensor blend_metric_counts_fwd_wrapper(
    ...
    CHECK_CUDA(fastgs_blend_config::debug, "blend_metric_counts_fwd")
...
blend_bwd_wrapper(
    ...
    torch::Tensor grad_projected_means_abs = torch::zeros(
        {n_primitives, 2},
        float_options
    );
    ...
    if (n_buckets > 0) {
        ...
        // implementation.
        fastgs_blend_backward_kernels::blend_backward_cu<<<n_buckets, 32>>>(
            ...
            reinterpret_cast<float2*>(
                grad_projected_means_abs.data_ptr<float>()
            ),
        ...
        CHECK_CUDA(fastgs_blend_config::debug, "blend_bwd")
    }
