...
renderCUDA(
	...
	const float* __restrict__ dL_dpixel_depth_sums,
...
)
{
	...
	float accum_rec[C] = { 0 };
	float dL_dpixel[C];
	float accum_color_square_rec = 0;
	float dL_dpixel_color_square = 0;
	float accum_coord_rec[3] = {0};
	float dL_dpixel_coord[3];
	float accum_t_rec = 0;
	float accum_depth_sum_rec = 0;
	float accum_depth_square_rec = 0;
	float dL_dpixel_t;
	float dL_dpixel_mt;
	float dL_dpixel_depth_sum = 0;
	float dL_dpixel_depth_square = 0;
	...
	if (inside) {
		...
		dL_dpixel_color_square = dL_dpixel_color_squares[pix_id];
		...
		if constexpr (GEO)
		{
			...
			if constexpr (DEPTH)
			{
				...
				dL_dpixel_depth_sum = dL_dpixel_depth_sums[pix_id];
				dL_dpixel_depth_square = dL_dpixel_depth_squares[pix_id];
			}
			if constexpr (NORMAL)
			{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				atomicAdd(
					...
					dchannel_dcolor * 2.f * c * dL_dpixel_color_square);
			}
			accum_color_square_rec = last_alpha * color_square + (1.f - last_alpha) * accum_color_square_rec;
			dL_dopa += (color_square - accum_color_square_rec) * dL_dpixel_color_square;
			...
			if constexpr (DEPTH)
			{
				...
				accum_depth_sum_rec = last_alpha * depth + (1.f - last_alpha) * accum_depth_sum_rec;
				accum_depth_square_rec = last_alpha * depth * depth + (1.f - last_alpha) * accum_depth_square_rec;
				dL_dopa += (depth - accum_depth_sum_rec) * dL_dpixel_depth_sum;
				dL_dopa += (depth * depth - accum_depth_square_rec) * dL_dpixel_depth_square;
				dL_dt = dpixel_t_dt * dL_dpixel_t;
				dL_dt += dchannel_dcolor / ln * dL_dpixel_depth_sum;
				dL_dt += dchannel_dcolor * 2.f * depth / ln * dL_dpixel_depth_square;
				if (contributor == max_contributor-1) {
					dL_dt += dL_dpixel_mt;
				}
...
void BACKWARD::render(
	...
	const float* dL_dpixel_depth_sum,
...
#define RENDER_CUDA_CALL(template_coord, template_depth, template_normal) \
    renderCUDA<NUM_CHANNELS, template_coord, template_depth, template_normal> <<<grid, block>>> ( \
        ...
        dL_dpixel_mdepth, dL_dpixel_color_square, dL_dpixel_depth_sum, dL_dpixel_depth_square, \
