...
def _split_sh_coefficients(
    ...
    if scene.feature.ndim != 3:
        raise ValueError(
            "faster_gs_native expects spherical harmonics with shape "
            f"(num_splats, sh_coeffs, 3); got {tuple(scene.feature.shape)}."
        )
    if scene.feature.shape[1] < 1:
        raise ValueError("faster_gs_native requires at least one SH basis.")
...
def _validate_inputs(scene: GaussianScene3D, camera: CameraState) -> None:
    if scene.center_position.device.type != "cuda":
        raise ValueError("faster_gs_native requires scene tensors on CUDA.")
    if camera.cam_to_world.device.type != "cuda":
        raise ValueError("faster_gs_native requires camera tensors on CUDA.")
    if camera.camera_convention != "opencv":
        raise ValueError(
            "faster_gs_native currently expects cameras in opencv convention; "
            f"got {camera.camera_convention!r}."
        )
    if scene.log_scales.shape[-1] != 3:
        raise ValueError(
            "faster_gs_native only supports 3D Gaussian scales with shape "
...
def render_faster_gs_native(
    ...
    *,
    ...
    if return_2d_projections:
        raise ValueError(
            "The faster_gs_native backend does not expose 2D Gaussian projections."
        )
    if return_projective_intersection_transforms:
        raise ValueError(
            "The faster_gs_native backend does not expose projective intersection "
            "transforms."
        )
