...
OptixTracer::traceBwd(uint32_t frameNumber,
    ...
    torch::Tensor rayGradVis           = torch::zeros({rayOri.size(0), rayOri.size(1), rayOri.size(2), 1}, opts);
    ...
    paramsHost.rayGradVis         = packed_accessor32<float, 4>(rayGradVis);
    ...
    return std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>(particleDensityGrad, particleRadianceGrad, rayGradVis);
}
