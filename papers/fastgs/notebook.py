...
with app.setup:
    ...
    FastGSDefaultName = Literal["garden_base", "garden_big", "garden_debug_val"]
...
@app.class_definition
class FastGSVanillaDensificationConfig(FastGSConfigBase):
...
class FastGSDensificationConfig(FastGSConfigBase):
    ...
    vanilla: FastGSVanillaDensificationConfig = Field(
        default_factory=FastGSVanillaDensificationConfig
    )
...
@app.cell
def _(_cell_40_COMPILED_FASTGS_L1_METRIC_MAP):
...
def _(compiled_fastgs_l1_metric_map):
    class FastGSVanillaDensification(BaseDensificationMethod):
        ...
        def bind(
            ...
            if not isinstance(family_ops, GaussianFamilyOps):
                raise TypeError(
                    "FastGSVanillaDensification requires GaussianFamilyOps."
                )
        ...
        def _accumulate_native_densification_info(
            ...
            if not isinstance(
                ...
                raise TypeError(
                    "FastGSVanillaDensification requires render outputs with "
                    "densification_info."
                )
            scene = context.state.model.scene
            densification_info = (
